...
extern "C" void doAPPay(const char *info)
{
...
{
    NSLog(@"9.0以后使用新API接口: %@ 。%@ 。", url, url.host);
    if([url.host isEqualToString:@"safepay"]){
        ...
        [[AlipaySDK defaultService] processOrderWithPaymentResult:url standbyCallback:^(NSDictionary *resultDic) {
            ...
            NSString *stringInt = [NSString stringWithFormat:@"%d",code];
            const char *char_content = [stringInt UTF8String];
            UnitySendMessage("GameManager", "StatusCallback", char_content);
        }];
    }
    return YES;
}
